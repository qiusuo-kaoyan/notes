...
function renderKaoyanDashboard() {
  ...
  titleEl.innerHTML = `<span class="title-icon">${currentAlert.emoji}</span><span class="title-text">考研复习战略仪表盘</span><span class="title-status" style="color: ${currentAlert.color};">${currentAlert.text}</span>`;

  // --- [V19 VICTORY FIX] 数据卡片渲染 (终极修正) ---
  const today = dv.date('today');

  // 1. 到期的错题
  const reviewMistakes = allMistakes.where(p => { const rd = dv.date(p.review_date); return rd && rd <= today; });

  // 2. 【核心升级】所有“待剖析”的例题，无论有无review_date，都视为今日待办
  ...
  // [V20 ARCHANGEL FIX] 模块 6: 高价值解法模型库 (终极形态)
  ...
  if (masteredExemplars.length > 0) {
    ...
    dv.table(
      ...
      masteredExemplars
        .sort(p => p.importance, 'desc') // 按重要性降序排列
        .map(p => [
          p.file.link,
