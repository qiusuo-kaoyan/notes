...
var TemplatesModal = class extends import_obsidian5.Modal {
  ...
  display() {
    return __async(this, null, function* () {
      ...
      contentEl.empty();
      contentEl.createEl("h4", { text: "添加/编辑模板" });
      new import_obsidian5.Setting(contentEl).setName(createFragment((e) => {
        e.createSpan({ text: "下面列表中的模板按优先级排序，顶部的模板将首先被替换。请注意" });
        e.createEl("code", { text: "a" });
        e.createSpan({ text: " > " });
        e.createEl("code", { text: "b" });
        e.createSpan({ text: "接着是" });
        e.createEl("code", { text: "b" });
        e.createSpan({ text: " > " });
        e.createEl("code", { text: "c" });
        e.createSpan({ text: "一起等同于" });
        e.createEl("code", { text: "a" });
        e.createSpan({ text: " > " });
        e.createEl("code", { text: "c" });
        e.createSpan({ text: "，但反之则不然。" });
      })).addButton((btn) => {
        btn.setTooltip("添加").setIcon("plus").onClick((event) => {
      ...
      if (this.plugin.settings.templates.length) {
        ...
        for (let i = 0; i < this.plugin.settings.templates.length; i++) {
          ...
          new import_obsidian5.Setting(item).setName(createFragment((e) => {
            ...
            return button.setTooltip("上移").setIcon("up-arrow-with-tail").onClick(() => __async(this, null, function* () {
              if (i != 0) {
            ...
            return button.setTooltip("下移").setIcon("down-arrow-with-tail").onClick(() => __async(this, null, function* () {
            ...
            return button.setTooltip("编辑").setIcon("edit").onClick(() => __async(this, null, function* () {
            ...
            return button.setTooltip("删除").setIcon("x").onClick(() => __async(this, null, function* () {
...
var TemplateSettingsModal = class extends import_obsidian5.Modal {
  ...
  display() {
    return __async(this, null, function* () {
      ...
      contentEl.empty();
      let replacedText;
      new import_obsidian5.Setting(contentEl).setName("匹配...").setDesc("要匹配和替换的字符串。不包括正则表达式。").addText((text) => {
        replacedText = text;
      ...
      let replacementText;
      new import_obsidian5.Setting(contentEl).setName("替换为...").setDesc("用于替换匹配项的字符串。不要转义反斜杠。").addText((text) => {
        replacementText = text;
        replacementText.setValue(this.template.replacement).onChange((current) => {
          this.template.replacement = current;
        });
      });
      new import_obsidian5.Setting(contentEl).setName("全局匹配").setDesc("匹配所有实例(而不仅仅是第一个)。").addToggle((toggle) => {
        toggle.setValue(this.template.globalMatch).onChange((current) => this.template.globalMatch = current);
      });
      new import_obsidian5.Setting(contentEl).setName("区分大小写").setDesc("匹配将区分大小写。").addToggle((toggle) => {
        toggle.setValue(this.template.sensitive).onChange((current) => this.template.sensitive = current);
      });
      new import_obsidian5.Setting(contentEl).setName("全词匹配").setDesc("仅全词匹配。").addToggle((toggle) => {
      ...
      new import_obsidian5.Setting(contentEl).addButton((b) => {
        b.setTooltip(this.create ? "Add" : "Save").setIcon("checkmark").onClick(() => __async(this, null, function* () {
          if (this.template.replaced == "") {
            new import_obsidian5.Notice("MathLinks: 请输入一个非空字符串进行替换");
          } else {
            if (this.create)
        ...
        b.setTooltip("取消").setIcon("cross").onClick(() => __async(this, null, function* () {
          this.close();
        }));
      });
    });
...
var ExcludeModal = class extends import_obsidian5.Modal {
  ...
  display() {
    return __async(this, null, function* () {
      const { contentEl } = this;
      contentEl.empty();
      contentEl.createEl("h4", { text: "排除的文件" });
      new import_obsidian5.Setting(contentEl).setName("此列表中的文件/文件夹将被 MathLinks 忽略。").addButton((btn) => {
        btn.setTooltip("添加").setIcon("plus").onClick((event) => {
...
var MathLinksSettingTab = class extends import_obsidian6.PluginSettingTab {
  ...
  display() {
    return __async(this, null, function* () {
      ...
      containerEl.empty();
      containerEl.createEl("h2", { text: "MathLinks 设置" });
      new import_obsidian6.Setting(containerEl).setName("模板").setDesc(createFragment((e) => {
        e.createSpan({ text: "使用模板生成数学链接。使用" });
        e.createEl("code", { text: "mathLink: auto" });
        e.createSpan({ text: "在文件中使用模板。" });
      })).addButton((button) => {
        return button.setButtonText("管理").onClick(() => __async(this, null, function* () {
      ...
      new import_obsidian6.Setting(containerEl).setName("排除的文件").setDesc("管理 MathLinks 将忽略的文件/路径。").addButton((button) => {
        return button.setButtonText("管理").onClick(() => __async(this, null, function* () {
      ...
      let prefix;
      new import_obsidian6.Setting(containerEl).setName("编辑块链接的前缀").setDesc(createFragment((e) => {
        e.createSpan({ text: "像这样的链接" });
        e.createEl("code", { text: "note#^block-id" });
        e.createSpan({ text: "将被渲染为" });
        ...
        toggle.setTooltip("禁用以忽略笔记名称。");
      });
      new import_obsidian6.Setting(containerEl).setName("在源代码模式下启用").setDesc("目前，在源代码模式下仅支持维基链接。").addToggle((toggle) => {
      ...
      new import_obsidian6.Setting(containerEl).setName("在大纲中渲染 MathJax").setDesc("在核心大纲视图中使用 MathJax 渲染标题。你需要重新加载应用程序以使此选项生效。").addToggle((toggle) => {
